...
const helpers = {
    ...
    crypto: {
        ...
        getRandomBinaryString: function (bits) {
            ...
            while (string === '') {
                let byteString = nanoid(bytes).toString('hex');
...
export const generateShares = (secret, shares, threshold)=>{
    ...
    let checksum = "";//sha256(hexSecret).digest('hex').substr(56);
    let checkedHexSecret = hexSecret + checksum;

    return Shamir.generateShares(checkedHexSecret, shares, threshold);
}
...
export const combineShares = (shares)=> {
    ...
    let secret = Buffer.from(checkedHexSecret, 'hex').toString('utf8');
    return secret;
}